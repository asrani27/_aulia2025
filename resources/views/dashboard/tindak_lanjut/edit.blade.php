    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 border border-white/20">
        <form action="{{ route('tindak_lanjut.update', $tindakLanjut) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="nomor" name="nomor" value="{{ old('nomor', $tindakLanjut->nomor) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="pemeriksaan_id" id="pemeriksaan_id"
                        ...
                        <option value="{{ $pemeriksaan->id }}" @if(old('pemeriksaan_id', $tindakLanjut->pemeriksaan_id)
                            == $pemeriksaan->id) selected @endif>
                            No : {{ $pemeriksaan->nomor ?? 'Tidak ada nama kegiatan' }}
            ...
            <div class="mt-6">
                ...
                <textarea name="uraian" id="uraian" rows="4"
                    ...
                    placeholder="Masukkan uraian temuan atau masalah"
                    required>{{ old('uraian', $tindakLanjut->uraian) }}</textarea>
                @error('uraian')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="B" @if(old('status', $tindakLanjut->status) == 'B') selected @endif>Belum
                            Diproses</option>
                        <option value="DP" @if(old('status', $tindakLanjut->status) == 'DP') selected @endif>Dalam
                            Proses</option>
                        <option value="S" @if(old('status', $tindakLanjut->status) == 'S') selected @endif>Selesai
                        </option>
                    </select>
                    @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
