...
<div class="max-w-7xl mx-auto">
    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 mb-6 border border-white/20">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 mb-2">Edit Tindak Lanjut</h1>
                <p class="text-gray-600">Form edit data tindak lanjut audit</p>
            </div>
    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 border border-white/20">
        <form action="{{ route('tindak_lanjut.update', $tindakLanjut) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" id="nomor" name="nomor"
                       value="{{ old('nomor', $tindakLanjut->nomor) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="pemeriksaan_id" id="pemeriksaan_id" 
                        ...
                        @foreach($pemeriksaans as $pemeriksaan)
                            <option value="{{ $pemeriksaan->id }}" 
                                    @if(old('pemeriksaan_id', $tindakLanjut->pemeriksaan_id) == $pemeriksaan->id) selected @endif>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal" 
                           ...
                           value="{{ old('tanggal', $tindakLanjut->tanggal->format('Y-m-d')) }}" required>
                    @error('tanggal')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="uraian" id="uraian" rows="4" 
                          ...
                          placeholder="Masukkan uraian temuan atau masalah" required>{{ old('uraian', $tindakLanjut->uraian) }}</textarea>
                @error('uraian')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="tindak_lanjut" id="tindak_lanjut" rows="4" 
                          ...
                          placeholder="Masukkan tindak lanjut yang akan dilakukan" required>{{ old('tindak_lanjut', $tindakLanjut->tindak_lanjut) }}</textarea>
                @error('tindak_lanjut')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <select name="status" id="status" 
                        ...
                        <option value="B" @if(old('status', $tindakLanjut->status) == 'B') selected @endif>Belum Diproses</option>
                        <option value="DP" @if(old('status', $tindakLanjut->status) == 'DP') selected @endif>Dalam Proses</option>
                        <option value="S" @if(old('status', $tindakLanjut->status) == 'S') selected @endif>Selesai</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3" 
                              ...
                              placeholder="Masukkan keterangan tambahan (opsional)">{{ old('keterangan', $tindakLanjut->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="rekomendasi_saran" id="rekomendasi_saran" rows="4" 
                          ...
                          placeholder="Masukkan rekomendasi atau saran (opsional)">{{ old('rekomendasi_saran', $tindakLanjut->rekomendasi_saran) }}</textarea>
