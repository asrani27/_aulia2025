...
<body>
    ...
    <!-- Report Title -->
    <div class="title">
        LAPORAN JADWAL TIM AUDIT INSPEKTORAT DAERAH
    </div>
    ...
    <div class="table-container">
        @if(isset($jadwalAudits) && $jadwalAudits->count() > 0)
        <table>
            <thead>
                <tr>
                    <th class="no-column">No</th>
                    <th class="nama-instansi-column">Nama Instansi</th>
                    <th class="alamat-column">Alamat</th>
                    <th class="id-tim-column">Nama Tim Audit</th>
                    <th class="tanggal-column">Tanggal Audit</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="nama-instansi-column">{{ $jadwalAudit->nama_instansi }}</td>
                    <td class="alamat-column">{{ $jadwalAudit->alamat }}</td>
                    <td class="id-tim-column">{{ $jadwalAudit->timAudit->nama_tim ?? '-' }}</td>
                    <td class="tanggal-column">{{ date('d-m-Y', strtotime($jadwalAudit->tgl_audit)) }}</td>
                    <td class="status-column">{{ $jadwalAudit->status }}</td>
                    <td class="keterangan-column">{{ $jadwalAudit->keterangan ?? '-' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        @else
        <div class="no-data">
            Tidak ada data jadwal audit untuk periode yang dipilih.
        </div>
        @endif
    </div>
