...
<div class="max-w-4xl mx-auto">
    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 mb-6 border border-white/20">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800 mb-2">Edit Jadwal Audit</h1>
                <p class="text-gray-600">Form edit jadwal audit</p>
            </div>
        </div>
    </div>
    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 border border-white/20">
        <form action="{{ route('jadwal_audit.update', $jadwalAudit->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                       id="nama_instansi" 
                       name="nama_instansi" 
                       value="{{ old('nama_instansi', $jadwalAudit->nama_instansi) }}"
            ...
            <div class="mb-6">
                ...
                <textarea id="alamat" 
                          ...
                          required>{{ old('alamat', $jadwalAudit->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="tim_audit_id" 
                    ...
                    <option value="{{ $timAudit->id }}" 
                            {{ old('tim_audit_id', $jadwalAudit->tim_audit_id) == $timAudit->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <input type="date" 
                       id="tgl_audit" 
                       name="tgl_audit" 
                       value="{{ old('tgl_audit', $jadwalAudit->tgl_audit->format('Y-m-d')) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
                       required>
                @error('tgl_audit')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-6">
                ...
                <select id="status" 
                    ...
                    <option value="Dijadwalkan" {{ old('status', $jadwalAudit->status) == 'Dijadwalkan' ? 'selected' : '' }}>Dijadwalkan</option>
                    <option value="Sedang Berlangsung" {{ old('status', $jadwalAudit->status) == 'Sedang Berlangsung' ? 'selected' : '' }}>Sedang Berlangsung</option>
            ...
            <div class="mb-6">
                ...
                <textarea id="keterangan" 
                          ...
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
                          placeholder="Masukkan keterangan tambahan (opsional)">{{ old('keterangan', $jadwalAudit->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
